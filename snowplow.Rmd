...
We will remove these using [regex and str_remove](https://cran.r-project.org/web/packages/stringr/vignettes/stringr.html). We will also trim any remaining whitespace off the ends.
...
The high number of NA streets may be due to the slight innacuracy in the GPS systems. Additionally, they plows may sometimes be on unmarked roads. 

It is suprising that Simon Dr. has the second most visits, considering it is quite small. Every other frequently plowed street is a major street, so why is Simon Dr. on here?
